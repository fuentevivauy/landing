...
type UploadField = 'thumbnail' | 'carousel';
...
export default function ProductModal({ isOpen, onClose, product, onSave, categories }: ProductModalProps) {
    ...
    const [uploadTarget, setUploadTarget] = useState<UploadField>('thumbnail');
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (product) {
                ...
                const baseSlug = formData.name.toLowerCase().trim().replace(/[\/\\_]+/g, '-').replace(/[^a-z0-9-]/g, '');
                    ...
                    <form id={'product-form'} onSubmit={handleSubmit} className={'space-y-6'}>
                        ...
                        <div className={'grid grid-cols-1 md:grid-cols-2 gap-6'}>
                                ...
                                <div className={'flex items-start gap-4'}>
                                    <div className={'w-24 h-24 rounded-xl bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center relative overflow-hidden'}>
                                        {formData.image_thumbnail ? (
                                            <img src={formData.image_thumbnail} alt={'Preview'} className={'w-full h-full object-cover'} />
                                        ) : (
                                            <>
                                    ...
                                    <div className={'flex-1'}>
                                        <button
                                            ...
                                            className={'inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-colors shadow-sm disabled:opacity-50'}
                                        >
                                            <Upload className={'w-4 h-4'} />
                                            {formData.image_thumbnail ? 'Cambiar' : 'Subir'}
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div className={'flex items-start gap-4'}>
                                    <div className={'w-24 h-24 rounded-xl bg-slate-50 dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 flex flex-col items-center justify-center relative overflow-hidden'}>
                                        {formData.image_carousel ? (
                                            <img src={formData.image_carousel} alt={'Preview'} className={'w-full h-full object-cover'} />
                                        ) : (
                                            <>
                                                <ImageIcon className={'w-6 h-6 text-slate-400 mb-1'} />
                                                <span className={'text-xs text-slate-400'}>Sin imagen</span>
                                    ...
                                    <div className={'flex-1'}>
                                        <button
                                            type={'button'}
                                            onClick={() => triggerFileInput('carousel')}
                                            disabled={isUploading}
                                            className={'inline-flex items-center gap-2 px-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg text-sm font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 cursor-pointer transition-colors shadow-sm disabled:opacity-50'}
                                        >
                                            <Upload className={'w-4 h-4'} />
                                            {formData.image_carousel ? 'Cambiar' : 'Subir'}
                                        </button>
                                    </div>
                                </div>
                            </div>
